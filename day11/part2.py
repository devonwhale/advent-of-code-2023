...
for galaxy in galaxy_locations:
    row = galaxy[0]
    column = galaxy[1]
    gap_to_insert = 999999
    additional_rows = len([x for x in empty_rows if x < row]) * gap_to_insert
    additional_columns = len([x for x in empty_columns if x < column]) * gap_to_insert
